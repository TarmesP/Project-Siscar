    ...
    public class Marcas
    {
        ...
        */
        ...
        public void alterar(int veiID2, Marca item)
        {
            ...
            {
                string query = "UPDATE tblmarcar SET MARNOME = @MARNOME, MAROBSERVACOES = @MAROBSERVACOES WHERE MARID = @MARID";
                SqlCommand cmd = new SqlCommand(query, conn);
                cmd.Parameters.AddWithValue("@MARNOME", item.nome);
                cmd.Parameters.AddWithValue("@MAROBSERVACOES", item.observacoes);
                cmd.Parameters.AddWithValue("@MARID", veiID2);
        ...
        */
        ...
        public void excluir(int veiID)
        {
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                string query = "DELETE FROM tblmarcar WHERE MARID = @MARID";
                SqlCommand cmd = new SqlCommand(query, conn);
                cmd.Parameters.AddWithValue("@MARID", veiID);
        ...
        */
        ...
        public Marca pesquisar(int varID)
        {
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                string query = "SELECT MARID, MARNOME, MAROBSERVACOES FROM tblmarcar WHERE MARID = @MARID";
                SqlCommand cmd = new SqlCommand(query, conn);
                cmd.Parameters.AddWithValue("@MARID", varID);
                ...
                if (reader.Read())
                {
                    return new Marca
                    {
                        codigo = (int)reader["MARID"],
                        nome = reader["MARNOME"].ToString(),
                        observacoes = reader["MAROBSERVACOES"].ToString()
                    };
                }
        ...
        */
        ...
        public List<Marca> ExibirTodos()
        {
            ...
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                string query = "SELECT MARID, MARNOME, MAROBSERVACOES FROM tblmarcar";
                SqlCommand cmd = new SqlCommand(query, conn);
